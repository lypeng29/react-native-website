---
id: version-0.57-sample-application-movies
title: 示例教程：电影列表
...
这里我们用`data`数据里面的id值来作为这个不重复key值。
解决警告："VirtualizedList: missing keys for items, make sure to specify a key property on an item or provide a custom keyExtractor"
